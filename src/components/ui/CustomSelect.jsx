...
export default function CustomSelect({ value, onChange, options, placeholder = 'Pilih...', required }) {
  ...
  return (
    <div className="relative w-full text-sm" ref={ref}>
      ...
      >
        <span className={selectedOption ? 'text-slate-800 dark:text-slate-100' : 'text-slate-500'}>
          ...
          {options.map((opt) => (
            <div
              ...
              className={`flex items-center justify-between px-4 py-3 cursor-pointer transition-colors ${
                value === opt.value
                  ? 'bg-zinc-100 dark:bg-zinc-700/50 text-slate-800 dark:text-slate-100 font-medium'
                  : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-zinc-700/30'
              }`}
            >
              <span>{opt.label}</span>
              {value === opt.value && <Check className="w-4 h-4 text-slate-800 dark:text-slate-100" />}
            </div>
          ))}
          {options.length === 0 && (
